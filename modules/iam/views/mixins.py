...
class OTPRequiredMixin(object):
    ...
    will be redirected to. Defaults to ``two_factor_login``.
    """
    ...
    def get_login_url(self):
        ...
        """
        return self.login_url and str(self.login_url) or reverse('two_factor_login')
        ...
        if not request.user.is_verified():
            if self.raise_unverified:
                ...
                return TemplateResponse(
                    request=request,
                    template='accounting/two_factor/core/otp_required.html',
                    status=403,
                )
